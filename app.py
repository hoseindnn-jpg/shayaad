...
def create_game(owner_user_id, owner_chat_id):
    ...
    text = (
        ...
        f"{link}\n\n"
        "این لینک رو برای بقیه بفرست. هرکسی وارد بشه، ربات ازش اسم بازی می‌پرسه.\n\n"
...
def join_game_start(user_id, chat_id, game_code):
    ...
    send_message(
        chat_id,
        "خوش اومدی 😄\n\n"
        "یه اسم برای بازی انتخاب کن و همینجا بفرست.\n"
        "امتیازها و نتایج با همین اسم نمایش داده می‌شن."
    )
...
def start_new_round(game_id, admin_user_id, admin_chat_id):
    ...
    question_text = (
        "🎲 دور جدید شروع شد!\n\n"
        f"سوال:\n<b>{q['question']}</b>\n\n"
        "حالا یه جواب بامزه، عجیب یا گمراه‌کننده بفرست؛ جوری که بقیه فکر کنن جواب درست همونه 😈\n\n"
...
def handle_start(message, args):
    ...
    text = (
        "سلام 😄\n"
        "من دستیار بازی جواب‌های گمراه‌کننده‌ام.\n\n"
        "اگه مدیر بازی هستی، می‌تونی بازی جدید بسازی.\n"
        "اگه بازیکنی، باید با لینک عضویت مدیر وارد بازی بشی."
    )
